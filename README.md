...
(<img width="1245" height="705" alt=" - visual selection-3" src="https://github.com/user-attachments/assets/fd1f3530-915a-466e-840c-9b42b7fafc27" />
)
...
|---|---|---|
| 1 | **Input Agent** | Reads source files and prepares a clean list of targets. |
| 2 | **Discovery Agent** | Resolves business names or addresses into target URLs using a search engine. |
| 3 | **Scraping Agents** (3 in parallel) | Fetch data in coordinated waves. |
| 4 | **Recovery Agent** | Detects blocks, rotates credentials, retries failed pages. |
| 5 | **Enrichment Agent** | Fills missing fields from secondary sources. |
| 6 | **Output Agent** | Cleans, deduplicates, and exports formatted Excel files. |
...
```bash
git clone https://github.com/<pragyamittal0>/multi-agent-data-pipeline.git
...
The current pipeline is a strong foundation. Planned improvements:
...
- **AI-Assisted Field Extraction** — Language model agent for sentiment, topics, and structured extraction from free text
...
Built using the multi-agent design pattern from distributed systems theory, applied to the practical problem of large-scale public data collection.
